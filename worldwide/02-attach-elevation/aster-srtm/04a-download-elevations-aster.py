...
elevations_path = config['elevation_aster_elevations_path']
...
for country, group in urls.groupby('country'):
    ...
    for url in group['url']:
        ...
        result = [result['astergdem'] for result in response_json['geonames']]
...
# add elevation as an attribute to the node IDs, replace nulls (-9999) with nan
nodes['elev'] = results
nodes['elev'] = nodes['elev'].replace(-9999, np.nan)
